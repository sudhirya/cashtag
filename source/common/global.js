...
g.formatCurrencyUnd = function (price, format = '00') {
  ...
  const und = Math.floor(price * 100) * 0.01 - dec;
...
function createIndexes(alphaIndex, charIndex) {
  ...
  while (i--) {
    ...
    const chars = e[1];
    const chr = chars[0];
    const addChar = (chr < 32 || chr > 126) || chr === 62 || chr === 60 || chr === 38 || chr === 34 || chr === 39;
    let charInfo;
    if (addChar) {
...
// Escape special characters in wrong format (that doesn't start with & or &)
g.escapeSpecialChars = function(str){
