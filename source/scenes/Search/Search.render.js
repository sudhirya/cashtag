  ...
  return class Render extends Component {
    ...
    _renderTabBar = () => (
      <View style={styles.tabbar} >
        ...
        { this._renderTabButton('#SHARED', 'SHARES') }
        { this._renderTabButton('#FIRE', 'FIRE') }
        { this._renderTabButton('#PLAYLIST', 'LISTS') }
      </View>
    )
    ...
    _renderDiscoverDesc = () => (
      <View style={styles.no_list} >
        <Text center line={40} size={16} color="#888" style={bs.mh_6x} >
          HIT{' '}<Image contain width={sizes.em(24)} height={sizes.em(40)} source={images.ic_live_audio_on} />{' '}TO SAVE SONGS{'\n'}
          OR{' '}<Icon name="fa diamond" color="#fff" size={24} />{' '}TO FOLLOW ARTISTS{'\n'}
          GET{' '}<Text bold size={sizes.em(22)} color="#fff">#</Text>{' '}FOR FUTURE STREAMZ
        </Text>
      </View>
    )
    _renderFireDesc = () => (
      <View style={styles.no_list} >
        <Text center line={40} size={16} color="#888" style={bs.mh_6x} >
          ADD{' '}<Image contain width={sizes.em(24)} height={sizes.em(40)} source={images.ic_fuego} />{' '}TO TRACKS{'\n'}
          OR LIVE SNAPSHOTS{' '}<FIcon name="camera" size={24} color="#fff" />{'\n'}
          DRIVES{' '}<Text embeded bold >TV</Text>{' '}<Image contain width={sizes.em(24)} height={sizes.em(40)} source={images.ic_header_tv} />{' '}RANKINGS
        </Text>
      </View>
    )
    _renderStreamDesc = () => (
      <View style={styles.no_list} >
        <Text center line={40} size={16} color="#888" style={bs.mh_6x} >
          HIT{' '}<Image contain width={sizes.em(24)} height={sizes.em(40)} source={images.ic_live_video_on} />{' '}TO STREAM{'\n'}
          DOUBLE TAP HANDLE{'\n'}
          [<Icon name="fd plus" color="#fff" size={20} />] CREATE MIXES #TAGS
        </Text>
      </View>
    )
    _renderPlaylistDesc = () => (
