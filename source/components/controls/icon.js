...
export default ({
  theme, name, size, sizeem, style, transparent, ...otherProps
}) => {
  let Component = FAIcon;
  let iconName = name;
  const iconSize = sizeem || sizes.em(size);

  const comps = [FAIcon, SLIcon, FDIcon, IOIcon, ETIcon, MCIcon, MTIcon, OTIcon, ZCIcon, EVIcon, FTIcon];
  const types = ['fa', 'sl', 'fd', 'io', 'et', 'mc', 'mt', 'ot', 'zc', 'ev', 'ft'];
  ...
  return (
    <Component name={iconName} size={iconSize} style={newstyle} {...otherProps} />
  );
};
